...
function BookingPage() {
    ...
    try {
      ...
      if (type == "Event") {
        const response = await axios.post(
          `http://localhost:5085/api/Volunteer/generate-event-url/${id}`,
          null,
          {
            headers: {
        ...
        const response = await axios.post(
          `http://localhost:5085/api/Volunteer/generate-session-url/${id}`,
          null,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const eventBookings = bookings?.filter(
    (booking) => booking?.bookingType === "Event"
  );
  const sessionBookings = bookings?.filter(
    (booking) => booking?.bookingType === "Session"
  );
  ...
  return (
    <div className="my-36 px-6 max-w-6xl mx-auto ">
      ...
      {bookings ? (
        <div>
          {(activeTab === "Event" ? eventBookings : sessionBookings)?.map(
            (booking) => (
              <div
              ...
              >
                ...
                {booking?.location === "Online" &&
                  (booking?.bookingStatus === "Pending" ||
                    ...
                    >
                      {generatingUrl === booking?.bookingID
                        ? "Generating..."
                        : "Generate Link"}
                    </button>
                  )}
